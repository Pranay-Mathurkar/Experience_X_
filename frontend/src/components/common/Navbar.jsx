...
export function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${scrolled ? 'bg-white/70 backdrop-blur-xl border-b border-purple-100/50 shadow-sm py-3' : 'bg-white/50 backdrop-blur-md border-b border-transparent py-5'}`}>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate('/')}>
              <div className="relative flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-500/30 group-hover:shadow-purple-500/50 group-hover:scale-105 transition-all duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="text-white/90">
            ...
            <div className="hidden md:flex items-center gap-4">
              {!isLoggedIn ? (
                <>
                  <button onClick={() => navigate('/login')} className="px-5 py-2 text-sm font-semibold text-slate-600 hover:text-purple-700 transition-colors">
                    Log in
                  </button>
                  <button onClick={() => navigate('/signup')} className="group relative inline-flex items-center justify-center px-6 py-2.5 text-sm font-semibold text-white transition-all duration-200 bg-slate-900 rounded-full hover:bg-slate-800 shadow-lg hover:shadow-xl hover:-translate-y-0.5">
                    Get Started
                  </button>
                </>
              ) : (
                <button onClick={handleLogout} className="px-6 py-2.5 text-sm font-semibold text-white bg-red-500 rounded-full hover:bg-red-600 shadow-md">
                  Logout
                </button>
              )}
            </div>
            ...
            <div className="md:hidden">
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 text-slate-600 hover:text-purple-600 transition-colors rounded-lg hover:bg-purple-50">
                {isMobileMenuOpen ? (
